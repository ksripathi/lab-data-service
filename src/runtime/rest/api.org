...
def update_and_delete_experiment(id):
    if request.method == 'GET':
        try:
            experiment = SystemInterface.get_experiment_by_id(id)
    ...
    if request.method == 'DELETE':
        ...
        try:
            q1 = SystemInterface.delete_experiment(id)
            return jsonify({"status":"sucess"})
...
class TestUpdateExperiment(TestCase):
    ...
    def test_update_experiment(self):
        ...
        payload1 = {'exp_name': 'Computer Programming',
                   'exp_id': 'cse02',
                   'key': KEY}

        headers = {'Content-Type': 'application/json'}

        response = self.client.post("/experients", data=json.dumps(payload),
                                 headers=headers)
...
class TestDeleteExperiment(TestCase):
    ...
    def test_delete_experiment(self):
        ...
        payload = {'exp_name': 'Data structures',
                   'exp_id': 'cse02',
                   'key': KEY
                  }
        ...
        response = self.client.post("/experiment", data=json.dumps(payload),
                                 headers=headers)

        response = self.client.delete("/experiment/1", headers=headers)
